...
- [~] **Phase 20-16: Bakery parser-gap closures (segmenter + prelude/root) — REFRAMED 2026-05-18, IN PROGRESS** — Original scope (#140–#144 incl. D-01 binding resolution) was reframed after the Task-1 Lokāyata HARD GATE fired **4 times**, each converting an inferred assumption into an observed fact at zero wrong-code cost (full audit trail: `.planning/phases/20-musician-timeline/20-16-OBSERVATIONS.md`). **Wave 0 SHIPPED on branch `feat/20-16-parity-hardening`** (commit `ff93c65`): `splitTopLevelStatements` made ASI/comment-aware — #148 leading-dot chain continuation (PV49-routed forward-peek), #150 binding-RHS-on-next-line (`=` backward-peek), #151 comment-only phantom segment (flush() strips `/* */`+`//`), #152 `/* */` block-comment skip. Editor 1564/1564 + parity-corpus 50/50 green, per-file loc-fidelity STOP gate vacuously green; banks 2/6 measured #141 repros (was 1/6) + likely more in the wider Bakery population. **Remaining 20-16 scope:** Wave B (#142 `samples({…})`/`github:`/`https:` strip-only — OQ2 observe-first whether the existing depth-walker already strips it; #143 guarded-boot `typeof X!=='undefined' && X(...)` second line-classifier), Wave C (#144 `( <string-literal> )` parenthesized-root arm), Verification (fresh `pnpm parity:bakery` PK17-step-6 re-measure vs 72.0% baseline; vendor #148/#150/#151/#152/#142/#143/#144 fixtures; SUMMARY + catalogues). **D-01 (#140/#141) REMOVED → Phase 20-17.** #149 + #147 stay backlog. CONTEXT/OBSERVATIONS in the phase dir.
