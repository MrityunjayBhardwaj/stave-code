...
- [x] **Phase 20-15: Strudel.cc parity hardening** — MERGED to main 2026-05-17 via PR #146 (merge `f6cb704`; post-merge gate verified the right way — HEAD==merge, 0/0 divergence, code grep-present, editor 1564/1564 + parity-corpus 50/50 green on truly-synced main). Issues #132/#134–#138 + #145 all closed (the 6 gap-class issues closed manually — GitHub honors only one issue per closing keyword). Branch deleted local+remote. All 6 gap classes closed (G1#134 γ-3, G2#135 α-1, G3#136 β-3, G4#137 α-4, G5#138 γ-2 + full D-01 timeline wiring, #132 β-2). **Real-world Bakery parity 4/10 (40%) → 72.0% (36/50, N=50 fresh Supabase, dated+SHA'd)**, independently re-verified. PV49 shared-walker substrate realized; PV51 (isSampleKey recursion) + P68 (tsup-watch-dies-on-DTS, root-caused & fixed via #145) catalogued. ≥9/10 known set; loc-fidelity 25/25 (pre-mortem drift provably absent); editor 1564/1564; 11/16 corpus byte-unchanged + 5 GOAL-permitted bare-Code→structured improvements. New friction → backlog #140–#145 (NOT fixed — scope discipline); γ-4 dropped (#140). Verifier verdict: GOAL ACHIEVED (2 pre-PR cleanups resolved). Original plan ↓ — A 2026-05-15 stress test of 10 live patterns from strudel.cc's Bakery (Supabase `code_v1` backend) measured **4/10 structurally parsed** — far below the curated corpus's 15/16, because real community code is messier than canonical examples. The misses decomposed into 5 named, reproducible parser-gap classes plus the deferred recursive-args gap from 20-14. This phase closes them. **Gaps:** G1 top-level `let`/`const` bindings + variable references (#134, 2/10, highest frequency, hardest — needs a mini symbol table / inline expansion); G2 `setcpm` and the `set*` family missing from `stripParserPrelude`'s skip set (#135, trivial ~1-line + the α-6 settingPatterns audit as authoritative source); G3 backtick template-literal string args incl. multi-line mini-notation (#136, root-matcher + bare-string arm, `${}`-interpolation → graceful fallback); G4 comment-only lines between `stack()` args (#137, arg-splitter doesn't strip interior `//` lines); G5 named-label `name: pattern` syntax (#138, generalize `extractTracks`'s `$:` regex with `:`-ambiguity false-positive guards — wires named tracks directly into the PR #116/#119 trackId/slot substrate); plus #132 recursive parsing inside `note`/`n`/`s` args (nested mini-with-chain — the arpoon residual). **Three waves:** α cheap-wins + shared substrate (G2 skip-set; extract a shared `skipWhitespaceAndLineComments(src,pos)` walker used by prelude + chain + arg-splitter per the γ-SUMMARY vyapti candidate; G4 consumes it); β root-matcher extensions (G3 backtick + #132 recursive args — same `parseRoot` surface, bundle); γ statement-level (G5 named labels via generalized `extractTracks` + false-positive matrix; G1 binding map + inline expansion — hardest, last). **Verification:** promote the 10-pattern Bakery probe to a permanent regression corpus subset (the 6 failing fixtures vendored alongside the 16 canonical tunes); target ≥9/10 structured; all existing 15/16 + corpus snapshots unchanged. **Out of scope:** template-literal `${}` interpolation evaluation (real JS — Code-fallback is correct), function/arrow-fn bindings, destructuring binds, a full JS parser (the structural matcher stays a matcher). **Inheritable substrate:** the shared whitespace/comment walker (3 existing callers + future splitArgs-style walkers); the binding map (reusable by any future "resolve references" / macro-expansion feature); named-label→trackId wiring (the timeline most wants this — label IS the track name, no `.p()` needed). Depends on 20-14 (α/β/γ merged — builds directly on the prelude/bare-string/chain parser work). See issues #132, #134, #135, #136, #137, #138 + the 2026-05-15 Bakery stress-test note in `memory/project_phase_20_musician_timeline.md`.

- [x] **Phase 20-16: Bakery parser-gap closures (segmenter + prelude/root) — REFRAMED 2026-05-18, COMPLETE LOCAL (PR open, not merged)** — Wave B/C + verification executed on `feat/20-16-parity-hardening`: **#142 fixture-only** (B-1 OQ2 — observed via vite-node that the existing `stripParserPrelude` depth walker is already shape-agnostic over `{`/`[`/`(`; NO code change), **#143** `GUARDED_BOOT_RE` second line-classifier (B-2), **#144** `parenStrMatch` parseRoot arm (C-1, PV49-routed inner offset). **V-1 fresh `pnpm parity:bakery --n 50` re-measure = 80.0% (40/50, N=50, 2026-05-18T14-34-02-237Z, f73b3956) = +8.0 pp over the 72.0% baseline.** V-2 vendored 7 permanent fixtures (verbatim issue bodies / Task-1 slices) + BAKERY-FIXTURES.md provenance. V-3 per-file loc-fidelity STOP gate **PASS** (552 insertions / 0 deletions vs the Wave-0 `ff93c65` baseline — purely additive, zero drift on every parity-unchanged file; parity-changed set = exactly the 7 fixtures, A-3 list ∅ since D-01 OUT). editor 1564/1564, app 347→361/361. 1 GENUINE NEW class → backlog **#153** (multiple sibling bare top-level statements); `-7LU6zgzViSM` = classifier-ordering artifact over a D-02 arrow-fn (NOT a regression). SUMMARY + vyapti (PV49 20-16 span addendum) + krama (PK16 stage-1 + PK17 this-cycle numbers) updated; no hetvabhasa (nothing >1 attempt). Single non-stacked PR open → main, `closes #142 #143 #144` (#148/#150/#151/#152 already closed by `ff93c65`, NOT re-closed; D-01 #140/#141 explicitly OUT → Phase 20-17). #149 + #147 stay backlog. **Original framing (kept for trail):** scope (#140–#144 incl. D-01) was reframed after the Task-1 Lokāyata HARD GATE fired **4 times** (audit trail: `20-16-OBSERVATIONS.md`); Wave 0 (`ff93c65`) made `splitTopLevelStatements` ASI/comment-aware (#148/#150/#151/#152). CONTEXT/OBSERVATIONS/SUMMARY in the phase dir.
- [x] **Phase 20-17: D-01 binding resolution — pervasive-context substitution + least-fixpoint — MERGED PR #157 (2026-05-19, merge `e3e6571`, base main, artifact-verified)** — Shipped: pervasive optional-arg `bindings?` threading (Wave A byte-identical signature refactor) + G1 chain-arg + G2 bound-ident-root arm + G3 additive `Code.via {literal:true;raw}` union + `classifyLiteralRhs` + D-1c 14-site consumer audit (HIGH-sev MusicalTimeline guard) + bounded least-fixpoint + occurs-check terminal. **D-03 DUAL gate MET first try, no bar-lowering:** crit-1 AMENDED on EVIDENCE (orig `--LsnlgQ6osk` empirically falsified — its `az2` RHS opaque-by-SHAPE not binding-blocked; re-anchored to dual `_72eEl7NwK9e`+`_LHtBlF8peGC` STRUCTURED) + crit-2 fresh `parity:bakery --n 50` = **86.0%** (43/50, 2026-05-19T13-24-45-538Z, f73b3956) = +6pp over the 80.0% baseline. 2 PK18 re-poses, both resolved within-plan on EVIDENCE (Wave-C bakery-152 premise falsification → V-3 allow-list flag; Wave-E cascade-WHY falsification → D-03 re-anchor + Finding-A precedence fix + 2 stale-test updates). 9 commits; editor 1564→**1603/1603**, app 361→**367/367** (parity-corpus 33 + loc-fidelity 33). #140 auto-closed + #141 manual-closed (GitHub 1-keyword limit). Catalogued **PV53** (pervasive optional-arg threading + bounded least-fixpoint) + **PK16 amendment** + **PK17 row** + **P70** (cascade classification can be wrong about WHY a case is bareCode — 2 recurrences this phase). Dominant residual class (5/7 of N=50 fallbacks) → backlog **#155** (Phase 20-18); **#156** (1 uncategorised) backlog. CONTEXT/OBSERVATIONS/PLAN/SUMMARY in the phase dir. **Original framing (trail):** Wave A-0 (20-16) source-scanned the substitution surface and **falsified CONTEXT's "primitive already built, minimal fixpoint" premise**: `bindings` reaches only 2 narrow sites (`parseExpression:832` whole-expr ident, `parseRoot:1061` stack-arg loop). Real D-01 = threading `bindings` as a **pervasive parse context** through the full recursive descent — **G1** chain-arg (`parseStrudel.ts:1015` loose recursive arm omits the 4th `bindings` arg), **G2** root-ident (no bound-ident-root arm in `parseRoot`), **G3** literal-RHS opaque (`var numChords = 4` → bareCode → opaque fence; needs literal passthrough), **G4** method-arg (`applyChain` doesn't thread `bindings` — the deepest, a signature + recursion refactor with PV49 loc-additivity preserved at every site) — THEN the bounded least-fixpoint + occurs-check ON TOP. The 20-16 CONTEXT D-01 decision (least-fixpoint substitution / **Datalog discipline**: total, PTIME, order-independent, occurs-check-stratified; matcher line = substitution-never-evaluation; D-02 dissolved; opaque-RHS fence kept) **remains the valid north star** — only the host phase + scoping premise changed. The 20-16-OBSERVATIONS WAVE A-0 4-gap map is this phase's RESEARCH seed. Needs its own discuss→research→plan→check (the Task-1 HARD GATE prototype `/tmp/proto-d01-fixpoint.spec.ts` carries forward as the re-gate oracle). Closes #140/#141. Depends on 20-16 (Wave 0 segmenter must land first — D-01 is unreachable for the dominant class until segmentation is correct).
- [ ] **Phase 20-18: Chain-root recognition — Strudel signal/builder family as Layer 1/2 Pattern IR (issue #155)** — DISCUSSED 2026-05-19 (CONTEXT in phase dir; 4 LOCKED decisions). Closes the dominant 20-17 V-1 residual class (5/7 of N=50 fallbacks): chains whose ROOT is an unbound Strudel free-function builder (`irand(12).struct(...)`, `perlin.range(...)`, `sine.range(...)`) — opaque-by-SHAPE, not binding-blocked; D-01's matcher line cannot reach it. **Layer correction (Ground-Truth-grounded during discuss):** these are `@strudel/core` `signal.mjs` pattern-level signals/builders = **Layer 1/2 Pattern IR / substrate axis 1** ("parser models what users type") — DISTINCT from Phase 24's Layer-3c `Synth`/`UGenGraph` synthesis (axis 5c, unchanged & not re-scoped). `collect.ts` already partially Ground-Truths `signal.mjs` (the RNG-chain work). **LOCKED:** D-01 curated closed-set ships the gate + flagged-general fallback measurement-only (never gate-counted); D-02 GENUINE semantic modelling — the full signal/builder IR-tag family, not an opaque wrapper (multi-wave; matcher stays a matcher — model structure never evaluate); D-03 dual gate, NO bar-lowering — RECLAIM `--LsnlgQ6osk` STRUCTURED in production (the case 20-17 re-anchored away from) AND fresh `parity:bakery --n 50` ≥ **90.0%** (from 86.0%); D-04 distinct sibling layer, Phase 24 unchanged. P70 PLANNING DIRECTIVE: RESEARCH must RUN all 7 V-1 residual samples through the parser (do NOT infer 5/7 from the `az2` exemplar — that is the P70 trap). Baseline on main `e3e6571`: editor 1603, app 367, Bakery 86.0% pin f73b3956. Backlog folded only if 7-sample audit proves same class: #156/#149/#147/#153. NEXT: `/anvi:plan-phase 20-18`.
