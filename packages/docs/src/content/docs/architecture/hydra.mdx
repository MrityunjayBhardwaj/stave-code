...
<Aside type="note" title="The DOM shim is smaller than p5's">
...
## Gotcha 1 — hydra swallows user errors *inside the library* — Stave re-surfaces them

A p5 `draw()` typo surfaces in the Stave Console. The hydra equivalent — a
...
**The fix:** rather than fight the library, Stave patches the worker's
...
the squiggle lands. (The V8 *eval-wrapper* frame is a distinct stack shape —
...
The original draw-error-surfacing comment *implied* parity across kinds — an inference that
...
never reaches `ready` → that's the *fallback* path, different again.)
</Aside>
...
fragment-count-independent (the bridge measures dispatch, not fill).

<Aside type="tip" title="Don't mirror the p5 density assertion onto hydra">
Contrast p5's line meshes, where the cost is CPU-side JS tessellation, so
...
**Verified by** `hydra-path-coverage.spec.ts` (the worker `gl()` accessor,
kind-agnostic `s.draw()` timing, the error re-surfacing, the flat-cost) and
