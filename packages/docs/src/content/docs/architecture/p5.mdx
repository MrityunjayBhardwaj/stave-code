...
<Aside type="tip" title="Main-canvas readback + trails now work worker == main">
This used to be the **transfer-clear limitation**: `transferToImageBitmap()`
is destructive — it moved the bitmap *out* of p5's render canvas and left it cleared, so
readback/trails went **silently blank** in the worker. Tier A removes it at the root:
...
arm now PAINTS). This makes the conditional-latch fallback unnecessary —
...
<Aside type="caution" title="A library's worker-DOM shim is BUILD-specific">
...
p5's `saveCanvas` calls `htmlCanvas.toBlob(cb)` (an `OffscreenCanvas` only has
...
ticked once by `feed.applyFrame`, and `onTick`/`__tick` is a no-op so the
...
frame. `setupDone()` flips `true` only after that wrapped setup runs
...
mesh — that cost is CPU, not pixels (the inverse of hydra's GPU-fill case).
...
<Aside type="caution" title="The fix: re-emit across the boundary, and split the channels">
...
error surfaced, no fallback). This is the per-thread-channel family — a per-thread channel is
invisible until explicitly bridged.
</Aside>
...
the same Console. The siblings caught up — hydra ([hydra](/architecture/hydra/),
its library swallows reactive-fn throws) and GLSL ([GLSL](/architecture/glsl/), a
...
**Verified by** `worker-draw-error.spec.ts` (user-throw surfacing),
`viz-density-lod.spec.ts` (density moves the cost curve, no remount), and the
