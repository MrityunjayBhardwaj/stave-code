...
<Aside type="caution" title="The blit is destructive — main-canvas readback blanks (#306/#316, P126)">
`transferToImageBitmap()` is zero-copy *because* it transfers the rendered bitmap
**out** of p5's render canvas and leaves it **cleared**. So a user sketch that reads
the main canvas back mid-`draw()` — `drawingContext.getImageData`, scrolling
waterfalls, trails that skip a full `background()` — sees nothing, and the failure is
**silent** (the `viz.worker` gauge still lights). The supported pattern is to
accumulate on an **owned `createGraphics()` buffer**, which is never transferred or
cleared (#308). Gate: `viz-worker-readback.spec.ts` (fills on the main thread, blanks
in the worker). A conditional non-destructive present that retires the limitation —
latch to a `createImageBitmap` copy on first readback, keeping zero-copy the default —
is planned in **#324**.
</Aside>
