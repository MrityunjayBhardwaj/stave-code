...
Only the canvas type and the audio source differ:
...
shader degrades to the proven `GLSLVizRenderer`.
...
<Aside type="note" title="Audio feed: no new seam. Event feed: a kind-specific marshal">
...
The worker draws only when the host calls `draw()` — there is no internal
...
<Aside type="tip" title="GLSL's draw can't throw — but its compile error is the user error, and it surfaces">
...
cannot throw post-mount — the *runtime* draw-error seam has nothing to
...
caught by the host → `onError` → `FallbackVizRenderer` degrades to main,
where the same shader throws again. That terminal catch in `GLSLVizRenderer` is
where it surfaces: `emitLog({ runtime: 'glsl', … })` to the main Console,
and — because a GLSL info log carries a `0:N` line — it maps that line back through
the wrapper preamble (`glslFragmentErrorUserLine`) to the user's **editor line**, so
the squiggle lands. All three runtimes now light a squiggle: p5 and hydra map their
thrown Error's stack back through the `new Function` header, GLSL maps its compiler
info-log line through the shader preamble. The error-surfacing triad: p5 (`draw()`
typo), hydra (library-swallowed, re-surfaced + line), GLSL
(mount-time compile) — each swallows differently, each surfaces *with a line*.
</Aside>
...
*worker* (`viz.worker > 0 && viz.glsl === 0`) exercising each seam: (a)
`glctx` accounting, (b) `viz.worker.draw` timing, (c) fallback on a
...
audio-reactive GLSL worker viz, all painting.
