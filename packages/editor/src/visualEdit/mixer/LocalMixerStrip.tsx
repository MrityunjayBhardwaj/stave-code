 ...
 *
 * It selects the cursor's strip out of the WHOLE-document strip set (not a
 * single-chunk build) so its `captureId` matches the engine's source-order
 * numbering — a single-chunk build would mis-number an anonymous `$:` as `$0`
 * and join the meter to the wrong track (GR1 / P-MIX-5). Writes ride the same
 * `applyToStrip(id, …)` path as the console, so a fader/pan drag is one undo and
 * goes live while playing (centralised Writeback re-eval).
 */
