  ...
  it('prices each candidate per document against the pinned production baseline', async () => {
    ...
    console.log(
      [
        ...
        '',
        '  rule                          moved  recovered(>1)  cap→1  newly-1  lost-correct  still-at-cap',
        ...priced.map(
          (p) =>
            `  ${p.id}  ${String(p.moved.length).padStart(5)}  ${String(
              p.recoveredReal.length,
            ).padStart(13)}  ${String(p.recoveredToOne.length).padStart(5)}  ${String(
              p.newlyPeriodOne.length,
            ).padStart(7)}  ${String(p.lostCorrectPeriod.length).padStart(12)}  ${String(
              p.stillAtCap,
            ).padStart(12)}`,
        ),
        '',
