  ...
  scene.lanes.forEach((lane, idx) => {
    ...
    for (const band of laneMarkBands(lane, box)) {
      ...
      for (const n of band.notes) {
        ...
        if (!r) continue
        drawLitMark(ctx, r, n.gain, theme, markShowsShape(lane, n, r, waveforms, pxPerCycle))
      }
    }
  })
}
 ...
 * a synth lane with a render draws it inside every bar, collapsed (#1740) or
 * expanded (#1745), and a sample mark draws its own file's slice (#1506).
 */
function markShowsShape(
  ...
  if (lane.envelope != null) return true
