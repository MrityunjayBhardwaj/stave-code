...
import { type SceneNote, type CollectedMarks, type TimelineScene } from '../timelineScene'
...
const voiceKeys = (scene: TimelineScene) =>
...
describe('applyStableVoiceOrder (#480)', () => {
  it('groupVoices reshuffles on clip swap — the bug being fixed', () => {
    ...
    expect(voiceKeys(sceneOf(analysis, 0, null, beforeSwap))).toEqual(['bd', 'hh', 'sn'])
    expect(voiceKeys(sceneOf(analysis, 0, null, afterSwap))).toEqual(['hh', 'bd', 'sn'])
  })

  it('pins voice order across a clip swap (drag or code-edit)', () => {
    const first = applyStableVoiceOrder(sceneOf(analysis, 0, null, beforeSwap), EMPTY_VOICE_ORDER)
    expect(voiceKeys(first.scene)).toEqual(['bd', 'hh', 'sn'])

    // Re-eval after the swap, threading the pinned order back.
    const second = applyStableVoiceOrder(sceneOf(analysis, 0, null, afterSwap), first.order)
  ...
  it('appends a genuinely new voice at the end (not alphabetical)', () => {
    ...
    const second = applyStableVoiceOrder(sceneOf(analysis, 0, null, withNew), first.order)
  ...
  it('leaves single-voice lanes untouched', () => {
    ...
    const built = sceneOf(analysis, 0, null, single)
