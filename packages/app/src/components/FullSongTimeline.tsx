...
export function FullSongTimeline(props: FullSongTimelineProps): React.ReactElement {
  ...
  // The TRUE song length — the rest span; also the playhead loop-wrap, the
  // note-mark collection window, and the bare-track implicit clip's end.
  const loopCycles = displaySpan(analysis)
  // While EXTENDING the last clip, the visual span temporarily grows past the
  // song so there's empty timeline to drag into (#487). null at rest →
  ...
  // Marks are collected over the TRUE song length (`loopCycles`) — collecting
  // over the padded extend span would repeat the looping pattern into the
  // trailing room instead of leaving it empty. The scene's transform basis still
  // uses the (possibly grown) `displayCycles` so the canvas and the component
  // agree (PV116, #487).
  const marks = useMemo(() => collectNoteMarks(props.ir ?? null, loopCycles), [props.ir, loopCycles])
  const scene = useMemo(() => buildTimelineScene(analysis, marks, displayCycles), [analysis, marks, displayCycles])
