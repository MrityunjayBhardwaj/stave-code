 ...
 * shell: ALL the worker logic (DOM shims, p5/hydra compile, bus feed, raw shims,
 * draw loop) lives in the bundler-agnostic `hostVizWorker` from the worker-safe
 * `@stave/editor/worker` subpath bundle. Keeping the entry here (in the app) is
 * what lets Next see the worker + bundle p5/hydra into the worker chunk; the editor
 * (tsup) stays bundler-agnostic.
 */
import { hostVizWorker } from '@stave/editor/worker'

// `self` is the DedicatedWorkerGlobalScope (has addEventListener + postMessage).
hostVizWorker(self as unknown as Parameters<typeof hostVizWorker>[0])
